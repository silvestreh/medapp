...
import { Application, ChatUser } from '../../declarations';

const userCache = new Map<string, { data: ChatUser; expiry: number }>();
...
export class Users {
  ...
  async get(id: Id, params?: Params): Promise<ChatUser> {
    const key = String(id);
    ...
    try {
      ...
      const user: ChatUser = response.data;
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        const minimalUser: ChatUser = { id: String(id), username: '' };
