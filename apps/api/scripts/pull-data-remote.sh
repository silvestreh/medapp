...
# running all mongo commands on the remote server via SSH:
# 1. A remote server's local MongoDB using mongoexport
# 2. A MongoDB Atlas cluster using mongoexport (executed on the remote server)
#
# Usage:
#   ./pull-data-remote.sh -u <mongodb_username> -w <mongodb_password> [-p <ssh_port>] [-m <mongodb_port>] -a <atlas_password> [-o <origin>]
#
# Required arguments:
...
#
# Example:
#   ./pull-data-remote.sh -u admin -w secret123 -p 2222 -m 27017 -a atlas_pwd123
#   ./pull-data-remote.sh -o atlas -a atlas_pwd123
#   ./pull-data-remote.sh -o do -u admin -w secret123 -p 2222 -m 27017
...
# Run mongoexport for each Atlas collection on the remote server via SSH
if [ -z "$ORIGIN" ] || [ "$ORIGIN" == "atlas" ]; then
    for collection in "${ATLAS_COLLECTIONS[@]}"; do
        echo "Exporting collection from Atlas (via remote server): $collection"
        ...
        echo "Atlas Command (executed on remote server):"
        echo "ssh $SSH_OPTS $REMOTE_HOST \"mongoexport \\
        ...
        # Execute mongoexport on the remote server for Atlas collections
        ssh $SSH_OPTS $REMOTE_HOST "mongoexport \\
        --uri=\"$ATLAS_URI\" \\
        --collection=\"$collection\" \\
        --jsonArray \\
