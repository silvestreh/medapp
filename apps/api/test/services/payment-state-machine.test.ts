...
} from '../../src/services/payments/payment-state-machine';
...
describe('payments state machine', () => {
  ...
  describe('appointment status', () => {
    it('allows the booking flow transitions', () => {
      assert.strictEqual(canTransitionAppointment('pending_payment', 'confirmed'), true);
      assert.strictEqual(canTransitionAppointment('pending_payment', 'expired'), true);
      assert.strictEqual(canTransitionAppointment('pending_payment', 'cancelled'), true);
      assert.strictEqual(canTransitionAppointment('confirmed', 'cancelled'), true);
      assert.strictEqual(canTransitionAppointment('expired', 'confirmed'), true);
    });

    it('refuses invalid transitions', () => {
      assert.strictEqual(canTransitionAppointment('cancelled', 'confirmed'), false);
      assert.strictEqual(canTransitionAppointment('confirmed', 'pending_payment'), false);
      assert.strictEqual(canTransitionAppointment('expired', 'pending_payment'), false);
    });
  });
});
